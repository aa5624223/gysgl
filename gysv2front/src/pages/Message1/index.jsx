...
export default class Message1 extends Component {
    ...
    SearchData = async (condition={},page={})=>{
        ...
        const result = await request(formData,'Home/getLw_News','POST');
        if(result.status===0){
            const {Lw_V_News,Lw_V_News_Count} = result.data;
            this.setState({dataSource:Lw_V_News,loading:false,dataTotal:Lw_V_News_Count,columns:G_News_columns(this),condition:Cur_condition})
        }else{
